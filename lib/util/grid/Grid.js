...
define(["jquery","Util","css!util/grid/Grid.css","css!modules/Font-Awesome/css/font-awesome.min.css"],function($,Util){
    ...
    Grid.prototype.render = function(){
        ...
        //Title栏面板
        $gridPanel.append('<div class="s_grid_title"></div>');
        //表格内容面板
        var $gridContent = $('<div class="s_grid_content"></div>');
        //操作栏面板
        $gridContent.append('<div class="s_grid_toolbar"></div>');
        //表头、表格数据存放面板
        $gridContent.append('<div><table class="table table-hover"><thead></thead><tbody></tbody></table></div>');
        //存放分页的面板
        $gridContent.append('<div class="s_grid_pagination"></div>');

        //hidden为true时默认隐藏表格
        config.hidden && $gridContent.css({"display":"none"});
           ...
           var index =  config.index=="checkbox" ? $(createIndex(config)).css({width:"20px"}).bind('click',function(event){
               ...
               if(target.nodeName.toLowerCase()=="input"){
                   $(target).parents("table").find("tr td:first-child :checkbox").each(function(index,element){
                       ...
                       //除去title行,修改Selected
                       index>0 && modifySelected(_this,index,element.checked);
                   });
               }
           }) : "<td width='20px'></td>";
        }
    ...
    Grid.prototype.renderGridContent = function(){
        ...
        for(var i= 0,row;row=data[i++];){
            ...
            $tr.bind('click',function(e){
                ...
                //从selected中添加或删除
                modifySelected(grid,trIndex,index.checked);
            });
     ...
     * @param flag          增加/删除
     */
    var modifySelected = function(grid,rowIndex,flag){
        if(flag){
