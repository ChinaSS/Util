...
define(["jquery","Util","css!util/grid/Grid.css","css!modules/Font-Awesome/css/font-awesome.min.css"],function($,Util){
    ...
    Grid.prototype.render = function(){
        ...
        //Title栏
        (config.title||typeof(config.hidden)!="undefined") && gridPanel.append(createTitle(this));

        var $gridContent = $('<div class="s_grid_content"></div>');
        //操作栏
        this.config.toolbar && $gridContent.append(createToolbar(this));
        //表格表头、表格数据
        this.pageInfo = getRealDataAdapter(this);
        var table = $('<table class="table table-hover"></table>').append(createHeadRow(config)).append(createDataRows(this));
        $('<div></div>').append(table).appendTo($gridContent);
        //分页,大于一页才渲染
        this.pageInfo.pageCount>1 && $gridContent.append(this.createPagination(config));
     ...
     * @returns $title
     */
    var createTitle = function(grid){
        var $title = $('<div class="s_grid_title">'+(grid.config.title || '数据列表')+'</div>');
        if(typeof(grid.config.hidden)!="undefined"){
            var $switch = $('<i class="fa"></i>');
            grid.config.hidden ? $switch.addClass("fa-chevron-circle-up") : $switch.addClass("fa-chevron-circle-down");
            //表格内容显示隐藏控制
        ...
        if(typeof(config.index)=="string"){
           var index =  config.index=="checkbox" ? $(createIndex(config)).css({width:"20px"}).bind('click',function(event){
               ...
               if(target.nodeName.toLowerCase()=="input"){
                   $(target).parents("table").find("tr td:first-child :checkbox").each(function(){
    ...
    var createDataRows = function(grid){
        ...
        for(var i= 0,row;row=data[i++];){
            ...
            for(var j=0,item;item=config.layout[j++];){
                ...
                $('<td></td>').append(tdValue).appendTo($tr);
            }
            //每行添加点击选中控制
